...
if [ "$1" = "admin" ]; then
    export USER=$(yq e .h2o.admin_username $PARAMS_YAML)
else
...
#    kubectl config use-context $SUPERVISOR_CLUSTER
    ...
    KUBECTL_VSPHERE_LOGIN_COMMAND=$(expect -c "
    spawn kubectl vsphere  login --server $SUPERVISOR_CLUSTER -u $USER --tanzu-kubernetes-cluster-name $TKC_NAME --tanzu-kubernetes-cluster-namespace $VSPHERE_NAMESPACE_NAME --insecure-skip-tls-verify
    expect \"*?assword:*\"
    send -- \"$VSPHERE_WITH_TANZU_PASSWORD\r\"
    expect eof
    ")
